...
static uint32_t DCD_WriteEmptyTxFifo(USB_OTG_CORE_HANDLE *pdev, uint32_t epnum)
{
  ...
  uint32_t len32b;
  txstatus.d32 = 0;
  uint32_t fifoemptymsk = 0;  //WXD Add
  ...
  if(ep->xfer_len == ep->xfer_count)  //WXD Add
  {
    /* Disable the Tx FIFO Empty(or Half Empty) Interrupt for this EP */
    fifoemptymsk = 1UL << ep->num;
	USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, fifoemptymsk, 0);
  }
