...
static int8_t SCSI_CheckAddressRange(uint8_t lun, uint32_t blk_offset, uint16_t blk_nbr, uint32_t total_blk_nbr);
...
static int8_t SCSI_ReadCapacity10(uint8_t lun, uint8_t *params)
{
	uint32_t SCSI_blk_nbr, SCSI_blk_size;
...
static int8_t SCSI_Read10(uint8_t lun , uint8_t *params)
{
	uint32_t SCSI_blk_addr, SCSI_blk_len;
	uint32_t  SCSI_blk_size, SCSI_blk_nbr;
	...
	{  
		...
		if( SCSI_CheckAddressRange(lun, SCSI_blk_addr, SCSI_blk_len, SCSI_blk_nbr) < 0)
		{
			my_printf("[%s] Error4!\n\r", __FUNCTION__);
			return -1; /* error */
		}
		...
		/* cases 4,5 : Hi <> Dn */
		if (MSC_BOT_cbw.dDataLength != (SCSI_blk_len*SCSI_blk_size))
		{
			SCSI_SenseCode(MSC_BOT_cbw.bLUN, 
						 ILLEGAL_REQUEST, 
						 INVALID_CDB);
		...
		usb_mas_read_memory(OPT_PREPARE, lun , SCSI_blk_addr , SCSI_blk_len);
	}
	else if(MSC_BOT_State == BOT_DATA_IN)
	{
		usb_mas_read_memory(OPT_ONGOING, lun , 0 , 0);
	}
	else
	{
		my_printf("[%s] Error6! MSC_BOT_State=%d\n\r", __FUNCTION__, MSC_BOT_State);
	}
...
static int8_t SCSI_Write10 (uint8_t lun , uint8_t *params)
{
	...
	if (MSC_BOT_State == BOT_IDLE) /* Idle */
	{
		...
		usb_mas_write_memory(OPT_PREPARE, lun, SCSI_blk_addr, SCSI_blk_len);
		MSC_BOT_State = BOT_DATA_OUT;  
	}
	else if(MSC_BOT_State == BOT_DATA_OUT)/* Write Process ongoing */
	{
		usb_mas_write_memory(OPT_ONGOING, lun, 0, 0);
//		return SCSI_ProcessWrite(lun);		
	}
	else
	{
		my_printf("[%s] Error7! MSC_BOT_State=%d\n\r", __FUNCTION__, MSC_BOT_State);
	}
...
static int8_t SCSI_CheckAddressRange (uint8_t lun , uint32_t blk_offset , uint16_t blk_nbr, uint32_t total_blk_nbr)
{
  ...
  if ((blk_offset + blk_nbr) > total_blk_nbr)
  {
