...
#define  UNI_OEM_DATA_ADDR  0x1000       //4KB offset
#define  UNI_OEM_DATA_SIZE  (87172*2)

int uni_oem_data_check(void)
{
//	my_printf("[%s] sizeof(uni2oem) = %u Byte\n\r", __FUNCTION__, sizeof(uni2oem));  //87172
//	my_printf("[%s] sizeof(oem2uni) = %u Byte\n\r", __FUNCTION__, sizeof(oem2uni));  //87172

	uint8_t __attribute__((aligned(2))) buff[512];
	uint16_t *sf_dat;
	uint32_t sf_addr, remain_len, check_len, idx, i, cnt;
	
	sf_dat = (uint16_t *)buff;
	
	//check uni2oem[]
	sf_addr = UNI_OEM_DATA_ADDR;
	remain_len = sizeof(uni2oem);
	idx = 0;
	while(remain_len > 0)
	{
		check_len = remain_len>sizeof(buff) ? sizeof(buff) : remain_len;
		sf_read_disk(sf_addr, buff, check_len);
		cnt = check_len/sizeof(uint16_t);
		for(i=0; i<cnt; i++)
		{
			if(sf_dat[i] != uni2oem[idx+i])
				return -1;
		}
		remain_len -= check_len;
		sf_addr += check_len;
		idx += cnt;
	}

	//check  oem2uni[]
	sf_addr = UNI_OEM_DATA_ADDR + sizeof(uni2oem);
	remain_len = sizeof(oem2uni);
	idx = 0;
	while(remain_len > 0)
	{
		check_len = remain_len>sizeof(buff) ? sizeof(buff) : remain_len;
...
int uni_oem_write_to_spi_flash(void)
{
	int ret;
	uint32_t write_addr, write_len;
	uint16_t *write_dat;
	
	write_addr = UNI_OEM_DATA_ADDR;
	write_len  = sizeof(uni2oem);
	write_dat = (uint16_t *)uni2oem;
	ret = sf_write_disk(write_addr, write_dat, write_len);
	
	write_addr = UNI_OEM_DATA_ADDR + sizeof(uni2oem);
	write_len  = sizeof(oem2uni);
	write_dat = (uint16_t *)oem2uni;	
	ret += sf_write_disk(write_addr, write_dat, write_len);

	return (ret==sizeof(uni2oem)+sizeof(oem2uni)) ? 0 : -1;
}

#if 0
//oem2uni[] and uni2oem[] store in SPI Flash
WCHAR ff_convert (	/* Converted code, 0 means conversion error */
	WCHAR	chr,	/* Character code to be converted */
	UINT	dir		/* 0: Unicode to OEM code, 1: OEM code to Unicode */
)
{
	...
	if (chr < 0x80) {	/* ASCII */
		...
		if (dir) {		/* OEM code to unicode */
			addr = UNI_OEM_DATA_ADDR + UNI_OEM_DATA_SIZE/2;
			hi = (UNI_OEM_DATA_SIZE/2) / 4 -1;
//			p = oem2uni;
//			hi = sizeof(oem2uni) / 4 - 1;
		} else {		/* Unicode to OEM code */
			addr = UNI_OEM_DATA_ADDR;
			hi = (UNI_OEM_DATA_SIZE/2) / 4 -1;
//			p = uni2oem;
//			hi = sizeof(uni2oem) / 4 - 1;
		}
