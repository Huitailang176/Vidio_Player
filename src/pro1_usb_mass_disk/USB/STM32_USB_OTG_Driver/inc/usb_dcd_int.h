...
//USBD_DCD_INT_fops结构的成员函数在usbd_core.c中实例化
//USBD_DCD_INT_fops结构的成员函数在发生各种USB中断后调用(usb_dcd_int.c), 
//    用于告知设备复位(Reset) 枚举(SetupStage) 收发数据(DataInStage DataInStage)
//    挂起或恢复(Suspend Resume) 连接状态(DevConnected DevDisconnected) SOF信号(SOF)等
