...
#define PLL_N      336             //change this to get high speed 336--168MHz 360--180MHz
...
//#endif
  ...
  /*
   * __Vectors是一个全局标号(变量),在启动文件中定义.
   * __Vectors的物理地址在链接时确定,由于STM32没有MMU,所以程序中标号的逻辑地址(虚拟地址)就是物理地址.
   * 根据ARM汇编语法,__Vectors标号的地址就是中断向量表的开始地址.
   * 这涉及ARM汇编,静态链接时符号的重定位, CPU异常/中断机制等(还是需要许多知识的)
   */
  extern uint32_t __Vectors;

  /*
   * 自动定位中断向量表. 无论是boot还是app,无论是Flash运行还是SRAM运行,
   * 也无论中断向量表是否有偏移都好用.
   */
